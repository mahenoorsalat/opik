.PHONY: cursor codex claude agent-configs check-agent-configs clean-agents help hooks hooks-remove agent-hooks agent-hooks-remove precommit precommit-all
...
help:
	...
	@echo "  make agent-configs       - Regenerate every opted-in surface (Claude/Cursor/Codex) for this tree"
	@echo "  make check-agent-configs - Detect agent-config drift (fails if .agents/ changed but not re-synced)"
...
# Refresh the opted-in agent-config surfaces (Claude/Cursor/Codex) for THIS tree
# from its own .agents/. The target the git hooks call.
agent-configs:
	@./scripts/sync-agent-configs.sh

# Detect drift without regenerating: warns/fails if .agents/ changed but wasn't
# re-synced, or if opted-in .claude/ output is missing in this tree. Runs in
# pre-commit and CI so stale skills can't silently ship.
check-agent-configs:
	@./scripts/agent-configs-manifest.sh check
