...
This is the trained counterpart to the [LLM judge](/guardrails/guardrails#llm-judge-guardrail): use the LLM judge to prototype a check with a natural-language policy, and once it matters, train a custom guardrail so it runs fast and cheap inline at high volume. It's also the trained counterpart to a [manual guardrail](/guardrails/guardrails#manual-guardrail): reach for a manual guardrail for a one-off heuristic, and train a custom guardrail once you want a real model backing that same check.
