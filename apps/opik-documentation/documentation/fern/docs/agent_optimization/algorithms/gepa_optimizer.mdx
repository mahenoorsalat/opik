...
- Reflection calls use `GepaOptimizer.model` and are budgeted separately from the metric budget: at most `max_reflection_calls` calls per run (default: `max_trials`). Each call is traced as an Opik span, and the run reports the total as `reflection_call_count` in the result details.
...
<ParamField path="max_reflection_calls" type="int" optional={true} default="max_trials">Budget for reflection-LLM calls — the calls GEPA makes to propose new candidates, which are not metric calls and are billed on top of them. Defaults to `max_trials`; set to `0` to remove the cap. A run that exhausts this budget stops with `finish_reason="reflection_budget"`.</ParamField>
