// This file was auto-generated by Fern from our API Definition.

import type * as OpikApi from "../../api/index.js";
import * as core from "../../core/index.js";
import type * as serializers from "../index.js";

export const CategoricalFeedbackDetailCreate: core.serialization.ObjectSchema<
    serializers.CategoricalFeedbackDetailCreate.Raw,
    OpikApi.CategoricalFeedbackDetailCreate
> = core.serialization.object({
    categories: core.serialization.record(core.serialization.string(), core.serialization.number()),
});

export declare namespace CategoricalFeedbackDetailCreate {
    export interface Raw {
        categories: Record<string, number>;
    }
}
