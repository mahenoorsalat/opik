...
```bash
pip install opik "google-adk>=1.3.0"
```

<Note>
  This integration requires **google-adk 1.3.0 or newer**. Opik raises an error on import if an
  older version is installed. If you cannot upgrade past 1.3.0, use `opik` 2.2.6 or earlier, which
  shipped a separate legacy tracer for those versions.
</Note>
